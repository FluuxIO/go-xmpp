...
func Test_NoStreamManagement(t *testing.T) {
	...
	config := Config{
		...
		Credential:             Password("test"),
		Insecure:               true,
		StreamManagementEnable: false} // Don't enable stream management
	...
	case <-time.After(3 * time.Second):
		t.Fatalf("timed out waiting for no stream management session. This probably means session process is broken")
	}
...
func Test_StreamManagementNoResume(t *testing.T) {
	...
	config := Config{
		...
		Insecure:               true,
		StreamManagementEnable: true,
		streamManagementResume: true} // Enable stream management. This should be overridden by the response from the server
...
//func Test_SendStanzaQueueNoSM(t *testing.T) {
//
